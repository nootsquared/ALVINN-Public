...
def process_live_feed(camera_index=0, threshold=0.5, stabilization_enabled=True):
    # ---- Load Models ----
    # Saliency Model
    ...
    print("  's' : Toggle depth stabilization")
    print("  'q' : Quit")
    ...
    while True:
        ...
        # Show threshold
        cv2.putText(overlay_frame, f"Threshold: {threshold:.2f}", (10, 90),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2, cv2.LINE_AA)
        ...
        # Display frame
        cv2.imshow("Saliency + Depth + Motion", overlay_frame)
    ...
    print(f"Session complete. {frame_count} frames processed in {total_time:.2f} seconds.")
    print(f"Average FPS: {frame_count / total_time:.2f}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Process live webcam feed with saliency, depth detection, and motion tracking')
    ...
    process_live_feed(args.camera, args.threshold, not args.no_stabilization)